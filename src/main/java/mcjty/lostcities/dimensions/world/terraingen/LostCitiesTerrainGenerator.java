    ...
    private void doCityChunk(int chunkX, int chunkZ, BuildingInfo info) {
        ...
        if (info.profile.isDefault()) {
            ...
            if (info.waterLevel > info.groundLevel) {
                ...
                for (int x = 0; x < 16; ++x) {
                    for (int z = 0; z < 16; ++z) {
                        driver.setBlockRange(x, info.groundLevel + 1, z, info.waterLevel, liquidChar);
                    }
                }
            }
        }
    ...
    private void generateBorder(BuildingInfo info, boolean canDoParks, int x, int z, BuildingInfo adjacent) {
        ...
        switch (info.profile.LANDSCAPE_TYPE) {
            case DEFAULT:
                ...
                setBlocksFromPalette(x, info.groundLevel - 6, z, info.getCityGroundLevel(), info.getCompiledPalette(), borderBlock);
                break;
            case SPACE: {
                ...
                if (adjacentY > 5) {
                    setBlocksFromPalette(x, adjacentY, z, info.getCityGroundLevel(), info.getCompiledPalette(), borderBlock);
                }
                break;
            }
            case FLOATING:
                setBlocksFromPalette(x, info.getCityGroundLevel() - 4, z, info.getCityGroundLevel() + 1, info.getCompiledPalette(), borderBlock);
                if (isCorner(x, z)) {
...
//        char defaultStreet = info.profile.isFloating() ? street2 : streetBase;
    ...
    private void setBlocksFromPalette(int x, int y, int z, int y2, CompiledPalette palette, char character) {
        if (palette.isSimple(character)) {
            ...
            driver.current(x, y, z);
            //Most of places considered y2 is included, but it actually not
            //so just make it included to prevent various bugs
            while (y <= y2) {
...
//                        clearRange(primer, index, lowestLevel, info.getCityGroundLevel() + info.getNumFloors() * 6, waterLevel > mainGroundLevel);
        ...
        if (info.floorsBelowGround > 0) {
            ...
            for (int x = 0; x < 16; x++) {
                ...
                setBlocksFromPalette(x, lowestLevel, 0, Math.min(info.getCityGroundLevel(), info.getZmin().getCityGroundLevel()), palette, fillerBlock);
                setBlocksFromPalette(x, lowestLevel, 15, Math.min(info.getCityGroundLevel(), info.getZmax().getCityGroundLevel()), palette, fillerBlock);
            }
            for (int z = 1; z < 15; z++) {
                setBlocksFromPalette(0, lowestLevel, z, Math.min(info.getCityGroundLevel(), info.getXmin().getCityGroundLevel()), palette, fillerBlock);
                setBlocksFromPalette(15, lowestLevel, z, Math.min(info.getCityGroundLevel(), info.getXmax().getCityGroundLevel()), palette, fillerBlock);
            }
        }
    ...
    private void generateDoors(BuildingInfo info, int height, int f) {
        ...
        if (info.hasConnectionAtX(f + info.floorsBelowGround)) {
            ...
            if (hasConnectionWithBuilding(f, info, info.getXmin())) {
                driver.setBlockRange(x, height, 6, height + 3, filler);
                driver.setBlockRange(x, height, 9, height + 3, filler);
        ...
        if (info.hasConnectionAtZ(f + info.floorsBelowGround)) {
            ...
            if (hasConnectionWithBuilding(f, info, info.getZmin())) {
                driver.setBlockRange(6, height, z, height + 3, filler);
                driver.setBlockRange(9, height, z, height + 3, filler);
    ...
    private void generateCorridorConnections(BuildingInfo info) {
        if (info.getXmin().hasXCorridor()) {
            int x = 0;
            for (int z = 7; z <= 10; z++) {
                driver.setBlockRange(x, info.groundLevel-5, z, info.groundLevel-3, airChar);
            }
        }
